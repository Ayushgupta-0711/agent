"""
Multi-Agent Data + Research Streamlit App
Features:
- Ingest CSV/Excel files, store in Pandas/SQLite, answer natural language data queries with charts & tables.
- Ingest PDF files (research papers), extract text, produce summaries, keywords, and basic Q&A.
- Coordinate 'Data Intelligence Agent' and 'Research Intelligence Agent' from a single chat-like interface.

How to run:
1. Install requirements: pip install -r requirements.txt
   Example requirements.txt contents (recommended):
     streamlit
     pandas
     openpyxl
     matplotlib
...
2. Run: streamlit run multi_agent_data_research_streamlit.py

Notes:
- This app uses local, explainable heuristics (TF-IDF retrieval + simple rule-based NL parsing).
- Optionally you can plug an LLM (OpenAI) for better Q&A by providing an API key in the UI (optional).
